...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $restaurant_name = filter_input(INPUT_POST, 'restaurant_name', FILTER_SANITIZE_SPECIAL_CHARS);
    ...
    $current_sales_revenue = filter_input(INPUT_POST, 'current_sales_revenue', FILTER_VALIDATE_FLOAT);
    $marketing_goals = filter_input(INPUT_POST, 'marketing_goals', FILTER_SANITIZE_SPECIAL_CHARS);

    if (empty($restaurant_name) || empty($owner_name) || empty($contact_email) || empty($contact_phone)) {
        $response['message'] = 'Please fill in all required fields.';
    } else {
        try {
            $sql = "INSERT INTO `business_applications` (`restaurant_name`, `owner_name`, `contact_email`, `contact_phone`, `current_sales_revenue`, `marketing_goals`) 
                    VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$restaurant_name, $owner_name, $contact_email, $contact_phone, $current_sales_revenue, $marketing_goals]);

            $response['success'] = true;
            $response['message'] = 'Application submitted successfully! We will review it shortly.';
            
        } catch (PDOException $e) {
            error_log("Business growth application submission error: " . $e->getMessage());
...
<head>
    ...
    <title>Grow Your Business - Sri Dish</title>
...
<body>
    <header>
        ...
        <div class="dropdown-menu">
            <ul>
                ...
                <li><a href="grow_business.php"><i class="fas fa-chart-line"></i> Grow Your Business</a></li>
    ...
    <main>
        <section class="business-form-section">
            <div class="form-card">
                <h2>Grow Your Business with Sri Dish</h2>
                <p>Tell us about your business and goals, and we'll help you create a growth plan!</p>
                <form id="business-form">
                    ...
                    <div class="form-group">
                        <label for="current-sales">Current Monthly Sales Revenue (LKR)</label>
                        <input type="number" id="current-sales" name="current_sales_revenue" step="0.01">
                    </div>
                    <div class="form-group">
                        <label for="marketing-goals">Your Marketing Goals</label>
                        <textarea id="marketing-goals" name="marketing_goals" rows="4" 
                        placeholder="e.g., Increase brand awareness, boost sales by 20%, attract new customers...">
                    </textarea>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch('grow_business.php', {
                        method: 'POST',
                    ...
                    if (data.success) {
                        alert("Application submitted successfully! Our growth team will contact you soon.");
                        form.reset();
                    } else {
